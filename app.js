...
// ║  Bypasses time locks, adds skip buttons, adds reset button  ║
...
function minsOf(hm)  { const [h, m] = hm.split(':').map(Number); return h * 60 + m; }
function afterTime(t){ return minsNow() >= minsOf(t); }
...
function unlocked(n) {
  switch (n) {
    ...
    case 4: return st.s3.done && (DEV_MODE || afterTime(STAGE4_TIME));
    case 5: return st.s4.done && (DEV_MODE || afterTime(STAGE5_TIME));
  }
  return false;
}
...
function lockedPreview(n) {
  ...
  const previews = {
    ...
    4: `${noiseDiv()}<div class="lock-preview"><span class="blk">██</span> ${corruptText('countries. Only one survives. The clues will lead you to',s)} <span class="blk">████████████████████</span><div class="lock-time">[ ${corruptText('unlocks at '+STAGE4_TIME+' after the trail',s+8)} ]</div></div>`,
    5: `${noiseDiv()}<div class="lock-preview"><span class="blk">████████████████████████████████████</span> <span class="blk">████████████</span> <span class="blk">████</span><div class="lock-time">[ ${corruptText('unlocks at '+STAGE5_TIME,s+2)} ]</div></div>`,
  };
  return previews[n] || '';
}
...
function openModal(idx) {
  ...
  img.style.background = MARKER_COLORS[idx];
  img.textContent = `📍 photo ${idx + 1} — replace with real image`;
...
// Re-check time-locked stages every 30 s in case the app is open across a boundary
